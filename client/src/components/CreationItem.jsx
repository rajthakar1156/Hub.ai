...
const CreationItem = ({item}) => {
  ...
  return (
    <div onClick={()=>setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-[#161617]  rounded-lg cursor-pointer'>
        <div className='flex justify-between items-center gap-4 '>
             <div className=''>
               <h2 >{item.prompt}</h2>
               <p className='text-gray-500'>{item.type} - {new Date(item.created_at).toLocaleDateString()}</p>
             </div>
             <button className='bg-[#4285F4]   text-white px-4 py-1 rounded-full'>
             {item.type}
             </button>
        </div>
        {
